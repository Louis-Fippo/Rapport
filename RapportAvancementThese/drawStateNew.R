...
drawStateNew<-function(PState, NState, Temps,typetrait)
{
  ...
  if(PState==NState)
  {
    ...
    #On trace le segment 
    segments(XDepart,YDepart,XArrive,YArrive,col=typetrait);
  }
  ...
  YInt=NState;
  #on change d'état
  segments(XDepart,YDepart,XInt,YInt,col=typetrait);
  ...
  #on trace l'etat
  segments(XInt,YInt,XArrive,YArrive,col=typetrait);
